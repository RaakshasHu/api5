...
def configure_ngrok():
    ngrok_token = "2pGjo6J3nt05NuzDLgMkPcCfx0g_XNSKpVRmRzxNMDyncCTe"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("binder5.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in binder5.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("binder5.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated binder5.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update binder5.txt on VPS: {str(e)}")
